...
EOS;
                        ...
                        try {
                            ...
                            if ($dbh == null) {
                                ...
                                $sql = 'SELECT t.task, g.genre, t.end_time, t.priority, t.memo FROM task t, genre g WHERE t.userId = ? and t.genre = g.id and t.status = 1 and t.start_time <= NOW() ORDER BY t.end_time ASC, t.priority DESC';
                                $stmt = $dbh->prepare($sql);
                                $stmt->execute(array($_SESSION['userId']));
                                
                                print "<table border=1><tr><th>タスク</th><th>ジャンル</th><th>締切</th><th>優先度</th><th>メモ</th></tr>";
                                
                                while ($result = $stmt->fetch(PDO::FETCH_ASSOC)) {
                                    print "<tr><td>".$result['task']."</td><td>".$result['genre']."</td><td>".$result['end_time']."</td><td>".$result['priority']."</td><td>".$result['memo']."</td>";
                                }
                                print "</table>";
                                ...
                                <br>
                                <a href="operation.php">タスク管理画面へ</a>
                                <br>
