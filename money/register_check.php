...
EOS;
                                    ...
                                    $sql3 = 'CREATE VIEW account_balance_'.$result['userId'].' AS SELECT acc.name, SUM(inc.amount) - SUM(exp.amount) FROM expense exp, income inc, account acc WHERE acc.userId = ? and acc.id = exp.account and acc.id = inc.account GROUP BY acc.id';
